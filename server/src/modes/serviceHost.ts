...
import { shadowSys } from './helpers/shadowSys';
...
currentScriptDoc.languageId ${localCurrentScriptDoc.languageId}`);
	...
	const host: ts.LanguageServiceHost = {
		...
		getDirectories: shadowSys.getDirectories,
		directoryExists: shadowSys.directoryExists,
		fileExists: shadowSys.fileExists,
		readFile: shadowSys.readFile,
		readDirectory: shadowSys.readDirectory,
		...
		resolveModuleNames(moduleNames: string[], containingFile: string): ts.ResolvedModule[] {
			...
			const ret = moduleNames.map(name => {
				...
				const resolved = ts.resolveModuleName(name, containingFile, compilerOptions, shadowSys).resolvedModule;
				if (!resolved) {
